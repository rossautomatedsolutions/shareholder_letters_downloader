...
def validate_output(output_path: Path) -> pd.DataFrame:
    ...
    print(f"year_range: {frame['year'].min()} -> {frame['year'].max()}")
    print("null_counts:")
    ...
    if list(frame.columns) != EXPECTED_COLUMNS:
        raise ValueError(
            f"Unexpected output columns: {list(frame.columns)}. Expected: {EXPECTED_COLUMNS}."
        )

    if frame.duplicated().any():
        raise ValueError("Generated sentiment stability output contains duplicate rows.")

    deviation_summary = frame.groupby("company_id", dropna=False)["sentiment_deviation"].mean()
    print("mean_deviation_by_company:")
    print(deviation_summary.head())

    if deviation_summary.dropna().empty:
        raise ValueError("Generated sentiment stability output has no non-null deviation values to validate.")
